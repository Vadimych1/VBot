...
class TrackedRobotIK(IK):
    def __init__(self, track_width, max_speed, wheel_r):
        ...
        self.max_speed = max_speed * (2 * np.pi / 60)
    ...
    def compute_wheel_speeds(self, v, w):
        ...
        v_left = v_left / self.wheel_r
        v_right = v_right / self.wheel_r

        scale = max(abs(v_left) / self.max_speed, abs(v_right) / self.max_speed, 1.0)
...
class MotorController:
    def __init__(
        self,
        kinematics: IK,
        serial_conn: serial.SerialTransport,
        max_rpm: int = 300,
        max_lin_speed=2,
        max_ang_speed=25.4,
    ):
    ...
    async def move_to(self, x, y, t=3, high_precision=False):
        ...
        result, rt = await self.rotate_to(
            angle, t=t / 3, high_precision=high_precision
        )  # rotate to (dx; dy) vector direction
        ...
        t = t - rt
        speed = max(
            self.max_lin_speed / 4, min(self.max_lin_speed / 6 * 5, distance / t)
        )
    ...
    async def rotate_to(self, w, t=1.5, high_precision=False):
        ...
        w_speed = max(
            self.max_ang_speed / 4, min(self.max_ang_speed / 3 * 2, abs(delta_w) / t)
        )
